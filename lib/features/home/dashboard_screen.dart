              ...
              delegate: SliverChildListDelegate([
                ...
                Row(
                  children: [
                    ...
                    StatCard(icon: Icons.account_balance_wallet_outlined, value: formatToman(state.moneySaved), label: 'ذخیره کردی', tone: NafasColors.money, soft: NafasColors.moneySoft),
                  ],
                ),
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _QuickAction(icon: Icons.calendar_month_rounded, label: 'تقویم نفس', tone: NafasColors.sky, soft: NafasColors.skySoft, onTap: () => Navigator.pushNamed(context, '/calendar'))),
                  const SizedBox(width: 10),
                  Expanded(child: _QuickAction(icon: Icons.edit_note_rounded, label: 'حال امروز', tone: NafasColors.sun, soft: NafasColors.sunSoft, onTap: () => Navigator.pushNamed(context, '/journal'))),
                ]),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _QuickAction(icon: Icons.emoji_events_rounded, label: 'دستاوردها', tone: NafasColors.money, soft: NafasColors.moneySoft, onTap: () => Navigator.pushNamed(context, '/achievements'))),
                  const SizedBox(width: 10),
                  Expanded(child: _QuickAction(icon: Icons.health_and_safety_rounded, label: 'کمک فوری', tone: NafasColors.rose, soft: NafasColors.roseSoft, onTap: () => Navigator.pushNamed(context, '/help'))),
                ]),
                const SizedBox(height: 10),
                _QuickAction(icon: Icons.history_rounded, label: 'ثبت لغزش بدون پاک شدن سابقه مسیر', tone: NafasColors.lilac, soft: NafasColors.lilacSoft, onTap: () => Navigator.pushNamed(context, '/slip')),
              ]),
            ),
          ),
        ],
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: const NafasDisclosureIcon(color: Colors.white),
            ),
          ],
        ),
      ),
          ...
          Row(
            children: [
              Container(
                width: 48,
                height: 48,
                decoration: BoxDecoration(color: NafasColors.moneySoft, borderRadius: BorderRadius.circular(16)),
                child: const Icon(Icons.flag_rounded, color: NafasColors.money),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
          ...
          Row(
            children: [
              Container(
                width: 40,
                height: 40,
                decoration: const BoxDecoration(color: NafasColors.lilacSoft, shape: BoxShape.circle),
                child: const Icon(Icons.eco_rounded, color: NafasColors.lilac, size: 20),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Row(
...
class _DailyInsight extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return NafasCard(
      onTap: onTap,
      backgroundColor: NafasColors.skySoft,
      child: Row(
        children: [
          Container(
            width: 58,
            ...
            child: Icon(hasCraving ? Icons.psychology_alt_rounded : Icons.favorite_rounded, color: NafasColors.sky, size: 28),
          ),
          ...
          const NafasDisclosureIcon(color: NafasColors.primary),
        ],
      ),
    );
  }
...
class _QuickAction extends StatelessWidget {
  const _QuickAction({required this.icon, required this.label, required this.tone, required this.soft, required this.onTap});
  final IconData icon;
  final String label;
  final Color tone;
  final Color soft;
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Container(width: 32, height: 32, decoration: BoxDecoration(color: soft, borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: tone, size: 18)),
