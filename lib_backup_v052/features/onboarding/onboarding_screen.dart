...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  @override
  void dispose() { goalController.dispose(); amountController.dispose(); reasonController.dispose(); super.dispose(); }
  ...
  Widget _usage() => _page(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 10),
      TextField(controller: reasonController, maxLines: 3, decoration: const InputDecoration(hintText: 'مثلاً برای سلامتی، خانواده، آرامش یا هزینه‌ها...')),
    ]),
  );
  ...
  Widget _quitDate() => _page(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _ChoiceTile(selected: DateTime.now().difference(quitDate).inHours.abs() >= 2, icon: Icons.calendar_month_rounded, title: 'قبلاً شروع کردم', subtitle: 'تاریخ شروع ترک رو انتخاب کن', onTap: () async {
        final picked = await showNafasJalaliDatePicker(context: context, firstDate: DateTime(2010), lastDate: DateTime.now(), initialDate: quitDate.isAfter(DateTime.now()) ? DateTime.now() : quitDate);
  ...
  Widget _goal() => _page(
    ...
    onTap: () {
      ...
      NafasScope.of(context).completeSetup(perDay: perDay, perPack: perPack, packPrice: packPrice, quitDate: quitDate, goal: goalController.text, goalAmount: amount, quitReason: reasonController.text);
    },
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
